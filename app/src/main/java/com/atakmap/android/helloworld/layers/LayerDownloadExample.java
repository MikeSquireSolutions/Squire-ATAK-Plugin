...
public class LayerDownloadExample extends BroadcastReceiver
    ...
    private void downloadLayer(Shape shape, double distM) {
        ...
        List<DatasetDescriptor> datasets = RasterUtils
                .queryDatasets(shape.getBounds(null), true);

        // Find the first available mobile imagery configuration
        ImageDatasetDescriptor mobac = null;
        for (DatasetDescriptor d : datasets) {
            if (d instanceof ImageDatasetDescriptor && d.getProvider().equals("mobac")) {
                mobac = (ImageDatasetDescriptor) d;
                break;
            }
        }
